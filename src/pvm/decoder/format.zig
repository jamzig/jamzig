...
pub fn formatInstructionWithArgs(
    ...
    switch (self.args) {
        ...
        .OneOffset => |args| try writer.print(" 0x{x}", .{args.offset}),
        .OneRegOneImm => |args| try writer.print(" r{d}, 0x{x}", .{ args.register_index, args.immediate }),
        .OneRegOneImmOneOffset => |args| try writer.print(" r{d}, 0x{x}, 0x{x}", .{ args.register_index, args.immediate, args.offset }),
        ...
        .TwoRegOneOffset => |args| try writer.print(" r{d}, r{d}, 0x{x}", .{ args.first_register_index, args.second_register_index, args.offset }),
