...
test "Correct parsing of all tiny test vectors" {
    ...
    var test_vectors = try buildTestVectors(allocator, "src/tests/vectors/safrole/safrole/tiny/");
...
test "Correct parsing of all full test vectors" {
    ...
    var test_vectors = try buildTestVectors(allocator, "src/tests/vectors/safrole/safrole/full/");
    defer test_vectors.deinit();
}
