...
pub const StringifyOptions = struct {
    /// When true, []u8 values are encoded as hex strings like "0x0000" instead of arrays
    emit_bytes_as_hex: bool = false,
                ...
                .pointer => |ptr_info| switch (ptr_info.size) {
                    ...
                    .many, .slice => {
                        ...
                        if (ptr_info.child == u8) {
                            ...
                            if (self.options.emit_bytes_as_hex) {
                                try self.valueStart();
                                try self.stream.writeByte('"');
                                try self.stream.writeAll("0x");
                                for (slice) |byte| {
                                    try std.fmt.formatIntValue(byte, "x", .{ .width = 2, .fill = '0' }, self.stream);
                                }
                                try self.stream.writeByte('"');
                                self.valueDone();
                                return;
                            } else if (!self.options.emit_strings_as_arrays and std.unicode.utf8ValidateSlice(slice)) {
